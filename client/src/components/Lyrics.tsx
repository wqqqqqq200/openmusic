...
export default function Lyrics({ lines, currentTime, onSeek, variant = 'center', size = 'default' }: Props) {
  ...
  return (
    ...
    >
      <div className={isSide ? (isLarge ? 'space-y-4 py-2' : 'space-y-3 py-2') : 'space-y-5 py-[40vh]'}>
        {visibleLines.map((line, i) => {
          ...
          const activeSideCls = isLarge
            ? 'text-white text-xl lg:text-2xl xl:text-3xl font-semibold leading-snug'
            : 'text-white text-base lg:text-lg xl:text-xl font-semibold leading-snug';
          const pastSideCls = isLarge
            ? 'text-white/20 text-base lg:text-lg xl:text-xl leading-snug'
            : 'text-white/20 text-sm lg:text-base xl:text-lg leading-snug';
          const futureSideCls = isLarge
            ? 'text-white/35 text-base lg:text-lg xl:text-xl leading-snug'
            : 'text-white/35 text-sm lg:text-base xl:text-lg leading-snug';
          ...
          return (
            ...
            >
              <p>{line.text}</p>
              {line.translation && (
                <p className={`mt-0.5 ${isLarge ? 'text-sm lg:text-base' : 'text-xs'} ${isActive ? 'text-white/60' : 'text-white/15'}`}>
                  {line.translation}
                </p>
              )}
            </div>
